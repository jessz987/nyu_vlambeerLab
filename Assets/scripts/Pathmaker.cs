...
//	DECLARE CLASS MEMBER VARIABLES:
	...
	private float rotateRightValue;

	void Start()
	{
		tileCounter = Random.Range(50, 150);
		rotateRightValue = Random.Range(0.05f, 0.15f);
	}
	

	void Update () {
		
		// destroy game object if globalTileCount > 500
		...
		// instantiate sphere and floor if counter < 50
		if (counter < tileCounter)
		{
			float randNum = Random.Range(0f, 1f);
			if (randNum < rotateRightValue)
			{
...
// - maybe randomize each pathmaker's lifetime? maybe randomize the probability it will turn right? etc.
//   if there's any number in your code, you can randomize it if you move it into a variable
...
// - add more detail to your original floorTile placeholder -- and let it randomly pick one of 3 different floorTile models, etc.
// 	 so for example, it could randomly pick a "normal" floor tile, or a cactus, or a rock, or a skull
